...
> 文档版本：2026-04-08  
> linger 版本：v0.2.0（Sprint 15 + A2/B2/A1 完成后）  
...
**已实现**：精确 LU 分解（`SparseLu<T>`，含列部分主元，Sprint 13）、精确 Cholesky（`SparseCholesky<T>`，稀疏左视，Sprint 14）、多波前 LU（`MultifrontalLu<T>`，Sprint 15）、`DirectSolver` trait、`DirectSolverPrecond` 包装器、RCM 重排序、COLAMD 重排序、**NodeNd 嵌套剖分**（纯 Rust，HEM 多层粗化 + BFS 双分 + FM 细化）、**迭代精化**（`DirectOptions.refine_steps`，SparseLu/SparseCholesky 均已激活）、**符号 Cholesky 预测**（`symbolic_cholesky`，精确 L 非零模式，消元树行子树算法）、`SolverBuilder` 统一接口（C2）、WASM 直接求解器 API（`WasmLuSolver` / `WasmCholeskySolver` / `WasmMultifrontalSolver`）。

**仍缺失**：外部直接法 FFI 绑定（SuperLU、STRUMPACK）、超节点 LU（无超节点聚合）、MPI 分布式直接法。
...
| **MMD_ATA** | ✓ | ✓ | ✗ | ✗ |
| **MMD_AT+A** | ✓ | ✓ | ✗ | ✗ |
| **METIS（嵌套剖分）** | ✗ | ✓ | ✓ | ✓（纯 Rust NodeNd，HEM+BFS+FM，无外部依赖）|
| **ParMETIS** | ✗ | ✓ | ✓ | ✗ |
| **SCOTCH** | ✗ | ✓（PT-Scotch）| ✓（串行 + PT-Scotch）| ✗ |
| **RCM（反 Cuthill-McKee）** | ✗ | ✗ | ✓ | ✓（纯 Rust，Sprint 13）|
| **几何排序（规则网格）** | ✗ | ✗ | ✓（GEOMETRIC）| ✗ |

> **对 FEA 的影响**：嵌套剖分（METIS）对非结构化 FEA 网格的填充减少效果远优于 COLAMD（3-10×），是大规模直接求解的关键。linger 现已实现 COLAMD、RCM（Sprint 13）和**纯 Rust NodeNd**（多层嵌套剖分，HEM 粗化 + BFS 双分 + FM 细化，无需 metis-sys 外部依赖）。
...
linger/
├── src/
│   ├── direct/                     # 直接求解器模块（Sprint 13–15 + A2/B2）
│   │   ├── mod.rs                   # DirectSolver trait + 重导出
│   │   ├── lu.rs                    # 纯 Rust 稀疏 LU（列部分主元 + 迭代精化）
│   │   ├── cholesky.rs              # 纯 Rust 稀疏 Cholesky（SPD，迭代精化）
│   │   ├── multifrontal.rs          # 多波前 LU + BLR 参数接口
│   │   ├── symbolic.rs              # 符号 LU/Cholesky 分析（精确非零模式预测）
│   │   ├── etree.rs                 # 消元树（elimination tree + col_counts）
│   │   ├── ordering/                # 重排序策略
│   │   │   ├── mod.rs               # OrderingMethod enum
│   │   │   ├── rcm.rs               # 反 Cuthill-McKee（纯 Rust）
│   │   │   ├── colamd.rs            # COLAMD（纯 Rust 移植）
│   │   │   └── nd.rs                # NodeNd 多层嵌套剖分（纯 Rust，无外部依赖）
│   │   └── triangular.rs            # 稀疏三角求解（前/回代）
│   ├── builder.rs                   # SolverBuilder 统一 API（C2）
│   └── wasm.rs                      # WASM 直接求解器 API（C1）
├── benches/
│   ├── bench_spmv.rs
│   ├── bench_krylov.rs
│   ├── bench_amg.rs
│   └── bench_direct.rs              # 直接法基准（A1）
└── ffi/
    ├── superlu/                     # 层次 B：SuperLU FFI（可选）
    └── strumpack/                   # 层次 C：STRUMPACK FFI（可选）
```
...
pub struct DirectOptions {
    ...
    pub refine_steps: usize,         // 迭代精化步数（0 = 不精化）
                                     // ≥1 时保存 A 的副本，每步计算 r=b-Ax，
                                     // 用已有因子求解 Aδx=r，更新 x+=δx
}
...
| `col_counts()` | `direct/etree.rs` | ✓ | 每列的 L 因子非零数（mark-and-sweep，Davis 2006）|
...
**目标**：用纯 Rust 实现 METIS NodeND 等效排序，消除外部依赖

| 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|
| `nd()` 公开 API | `direct/ordering/nd.rs` | ✓ | 与 `rcm`/`colamd` 相同接口 |
| HEM 多层粗化 | `nd.rs` 内部 | ✓ | Heavy-Edge Matching，target=60 |
| BFS 双极着色 | `nd.rs` 内部 | ✓ | 双端 BFS，找最远点对 |
| 顶点分隔符提取 | `nd.rs` 内部 | ✓ | 边界节点移入分隔符集 |
| FM 贪心细化 | `nd.rs` 内部 | ✓ | 最多 5 轮，balance_slack=0.20 |
| 递归嵌套剖分 | `nd.rs` 内部 | ✓ | 深度 ≤ 64，小图退回 AMD |
| `OrderingMethod::NodeNd` | `direct/ordering/mod.rs` | ✓ | enum 变体 + 三个求解器集成 |
| `Ordering::NodeNd` | `builder.rs` | ✓ | SolverBuilder 映射 |
| 集成测试 | `tests/test_ordering_nd.rs` | ✓ | 14 个集成测试 |
...
| 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|
| `WasmLuSolver` | `wasm.rs` | ✓ | `new(ordering)` + `solve(a, b)` |
| `WasmCholeskySolver` | `wasm.rs` | ✓ | 同上 |
| `WasmMultifrontalSolver` | `wasm.rs` | ✓ | 额外 `with_blr()` + `solve_precond_gmres()` |
| `SolverBuilder` | `builder.rs` | ✓ | fluent API，统一 Krylov + 直接法 |
| `SolveMethod`、`DirectBackend`、`PrecondChoice`、`Ordering` | `builder.rs` | ✓ | 枚举驱动配置 |
| `solve_auto()` | `builder.rs` | ✓ | 简单入口，自动选 Cholesky/MultifrontalLu |
| 集成测试 | `tests/test_builder_c2.rs` | ✓ | 15 个集成测试 |

---

#### A2 — 迭代精化激活【已完成】

**目标**：激活 `DirectOptions.refine_steps` 字段，在 SparseLu/SparseCholesky 的 solve() 中执行迭代精化

| 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|
| `SparseLu` 迭代精化 | `direct/lu.rs` | ✓ | solve() 末尾循环：r=b-Ax → Aδx=r（复用 L,U）→ x+=δx |
| `SparseCholesky` 迭代精化 | `direct/cholesky.rs` | ✓ | 同上，SPD 路径 |
| `a_stored` 字段 | 两文件 | ✓ | refine_steps>0 时保存 A 副本用于 SpMV |
| 集成测试 | `tests/test_refinement.rs` | ✓ | 7 个测试（基准/精化/多RHS/病态矩阵）|

---

#### B2 — 符号 Cholesky 精确模式预测【已完成】

**目标**：为稀疏 Cholesky 提供精确 L 非零模式预测，用于精确预分配因子存储

| 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|
| `symbolic_cholesky()` | `direct/symbolic.rs` | ✓ | 左视 DFS 达到集算法，O(nnz(L)) |
| `SymbolicCholesky` 结构 | `direct/symbolic.rs` | ✓ | CSC 格式 L 模式 + col_count + parent |
| `col_counts()` 修正 | `direct/etree.rs` | ✓ | 替换为正确的 mark-and-sweep 算法 |
| 公开导出 | `direct/mod.rs` | ✓ | `pub use symbolic::{SymbolicCholesky, symbolic_cholesky}` |
| 集成测试 | `tests/test_symbolic_cholesky.rs` | ✓ | 13 个测试（tridiag/2D/边界情况/一致性）|

---

#### A1 — 直接法性能基准【已完成】

**目标**：建立 `bench_direct.rs` Criterion 基准，量化各求解器和排序策略的实际性能

| 基准组 | 内容 | 说明 |
|--------|------|------|
| `SparseLu/factorize` | 1D/2D Laplacian，n=50-200 | 分解时间 vs 规模 |
| `SparseLu/solve` | 1D Laplacian，n=50-200 | 三角求解时间（因子已存）|
| `SparseCholesky/factorize` | 1D/2D，n=50-400 | 稀疏 Cholesky 分解 |
| `MultifrontalLu/factorize` | 1D，n=30-100 | 多波前 LU 分解 |
| `Cholesky/ordering` | 16×16 网格，4 种排序 | Natural/Rcm/Colamd/NodeNd 对比 |
| `SparseLu/refinement` | n=100，0-3 步精化 | 迭代精化时间开销 |
...
B1   ───── 纯 Rust NodeNd 嵌套剖分                        [HEM+BFS+FM，无 metis-sys] ✓ 已完成
C1/C2 ──── WASM 直接法 API + SolverBuilder               [wasm-bindgen + fluent API] ✓ 已完成
A2   ───── 迭代精化激活                                    [SparseLu + SparseCholesky] ✓ 已完成
B2   ───── 符号 Cholesky 精确模式预测                      [symbolic_cholesky，O(nnz(L))] ✓ 已完成
A1   ───── 直接法性能基准                                  [bench_direct.rs，6 组基准] ✓ 已完成
...
### 当前能力状态（v0.2.0，Sprint 15 + A1/A2/B1/B2/C1/C2 完成后）

| 能力域 | 状态 | 说明 |
|--------|------|------|
| 精确稀疏 LU | ✓ 已完成（S13/S15）| SparseLu（列部分主元）、MultifrontalLu（多波前）|
| 精确稀疏 Cholesky | ✓ 已完成（S14）| SparseCholesky（稀疏左视，O(nnz(L)) 内存）|
| 迭代精化 | ✓ 已完成（A2）| refine_steps 在 SparseLu/SparseCholesky 均已激活 |
| 符号 Cholesky 模式预测 | ✓ 已完成（B2）| symbolic_cholesky()，精确 L 非零模式，O(nnz(L))；col_counts() 已修正 |
| 列重排序（COLAMD/RCM）| ✓ 已完成（S13）| 纯 Rust，ordering::colamd / ordering::rcm |
| **嵌套剖分（NodeNd）** | ✓ 已完成（B1）| 纯 Rust，无 metis-sys 依赖，HEM+BFS+FM |
| 主元选取（部分主元）| ✓ 已完成（S13）| DirectOptions.pivot_threshold |
| 消元树 + 符号分析 | ✓ 已完成（S14/B2）| etree.rs + symbolic.rs，col_counts 已修正 |
| BLR 近似接口 | 部分已实现（S15）| MultifrontalLu 参数接口；标量波前精确计算 |
| **SolverBuilder 统一 API** | ✓ 已完成（C2）| builder.rs，Krylov + Direct + 排序 + 预条件器统一配置 |
| **WASM 直接求解器 API** | ✓ 已完成（C1）| WasmLuSolver / WasmCholeskySolver / WasmMultifrontalSolver |
| **直接法性能基准** | ✓ 已完成（A1）| benches/bench_direct.rs，6 组 Criterion 基准 |
...
1. **超节点聚合（可选）**：启用 MultifrontalLu 的 BLR 低秩压缩（当前参数接口已就绪，需超节点合并实现）
2. **大规模基准对比**：用 `benches/bench_direct.rs` 补充 10^4–10^5 DOF 数据，对比各排序策略内存/时间
3. **SuperLU FFI（可选）**：feature-gated `superlu-sys`，提供外部后端对比
4. **STRUMPACK FFI（可选）**：feature-gated，BLR/HSS 近似分解作为大规模预条件器
