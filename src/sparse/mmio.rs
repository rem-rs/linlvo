//! Matrix Market (.mtx) file reader/writer.
//!
...
//! Collection).
//!
//! Supported header variants (read):
...
//! 0-based on output.  The writer converts 0-based back to 1-based.
...
/// Write a CSR matrix to a Matrix Market file on disk.
///
/// Always writes in `coordinate real general` format (0-based indices are
/// converted to 1-based).  Structural zeros whose value is exactly `0.0` are
/// filtered out.
///
/// # Errors
/// Returns an [`MmioError::Io`] on any I/O failure.
pub fn write_matrix_market<P: AsRef<Path>>(path: P, a: &CsrMatrix<f64>) -> Result<(), MmioError> {
    let file = std::fs::File::create(path)?;
    let mut writer = io::BufWriter::new(file);
    write_matrix_market_impl(&mut writer, a)
}

/// Write a CSR matrix in Matrix Market format to a `String`.
///
/// Useful for testing without touching the filesystem.
pub fn write_matrix_market_str(a: &CsrMatrix<f64>) -> Result<String, MmioError> {
    let mut buf = Vec::new();
    write_matrix_market_impl(&mut buf, a)?;
    Ok(String::from_utf8(buf).expect("mmio output is always valid UTF-8"))
}

// ─── internal parsing ─────────────────────────────────────────────────────────

fn write_matrix_market_impl<W: Write>(w: &mut W, a: &CsrMatrix<f64>) -> Result<(), MmioError> {
    // Count non-structural-zeros.
    let nnz: usize = a.values().iter().filter(|&&v| v != 0.0).count();

    writeln!(w, "%%MatrixMarket matrix coordinate real general")?;
    writeln!(w, "% Generated by linger")?;
    ...
    for row in 0..a.nrows() {
        for ptr in row_ptr[row]..row_ptr[row + 1] {
            ...
            writeln!(w, "{} {} {:?}", row + 1, col_idx[ptr] + 1, val)?;
        }
    }
